...
| 维度 | 说明 |
|------|------|
| **场景** | 局域网内 DEMO 交互控制，PC 端应用程序远程操控 |
| **数据特征** | 数据量小，传输频率低（< 100 fps），指令/控制类型数据 |
| **协议理念** | XML 格式可读性高，可编辑，可调试；以 CRLF 为行边界 |
| **性能目标** | 支持每秒 60~100 次方法调用，延迟稳定（< 10ms） |
| **线程安全** | 服务端通过 `SynchronizationContext.Send` 将方法调用封送到 UI 线程，保证注册对象的线程安全 |
...
基类 `RPCServerBase` 以 **CRLF 数据行** 作为行单位，一个数据行内可包含多条子协议消息：
...
- **行层**（基类统一处理）：TCP 字节流 → CRLF 拆分 → 环形缓冲 → 提取数据行
- **消息层**（子类协议实现）：数据行字节 → `ParseInvokeMessage()` → `IEnumerable<InvokeMessage>`
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 680 400" width="100%" height="100%" style="max-width:680px;">
  ...
  <text x="340" y="80" text-anchor="middle" font-size="11" font-weight="bold" fill="#4a5568">数据行解析</text>
  <text x="340" y="98" text-anchor="middle" font-size="10" fill="#718096">环形缓冲 RingBuffer</text>
  <text x="340" y="114" text-anchor="middle" font-size="10" fill="#718096">CRLF 行拆分</text>
  ...
  <text x="340" y="165" text-anchor="middle" font-size="10" fill="#a0aec0">↓ 抽象方法：ParseInvokeMessage() / ConvertResponseMessage() ↓</text>
...
| **行解析层** | 环形缓冲 + CRLF 扫描 | 字节流 → CRLF 行拆分（基类统一实现） |
| **协议解析层** | `ParseInvokeMessage()` (abstract) | 行字节 → `InvokeMessage[]`（子类实现） |
| **调度层** | `CallInvokeMessageAsync()` + `SemaphoreSlim` | 并发调度、消息校验、参数转换 |
| **执行层** | `SynchronizationContext.Send()` + `MethodInfo.Invoke` | 在目标线程上安全执行注册对象方法 |
| **响应层** | `WriteResponseMessageAsync()` + `ConvertResponseMessage()` (abstract) | 序列化响应并写回客户端 |
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620 280" width="100%" height="100%" style="max-width:620px;">
  ...
  <text x="110" y="48" text-anchor="middle" font-size="9" fill="#4a5568">_ = CallInvokeMessageAsync()</text>
  ...
  <text x="110" y="108" text-anchor="middle" font-size="9" fill="#4a5568">_ = CallInvokeMessageAsync()</text>
  ...
  <text x="110" y="193" text-anchor="middle" font-size="9" fill="#4a5568">_ = CallInvokeMessageAsync()</text>
  ...
  <text x="330" y="115" text-anchor="middle" font-size="10" fill="#975a16">(24, 60)</text>
...
- **Fire-and-Forget**：`_ = CallInvokeMessageAsync(...)` 不阻塞读循环
- **信号量限流**：`SemaphoreSlim(24, 60)` 防止 ThreadPool 线程因 `Send` 阻塞而过度膨胀
- **SyncContext.Send**：保证注册对象的方法在构造线程（通常为 UI 线程）上执行
...
| 成员 | 类型 | 说明 |
|------|------|------|
| `NamedRegex` | `static Regex` | 对象/方法名称校验规则：`^[a-zA-Z_][a-zA-Z0-9_]*$` |
| `NewLine` | `static byte[]` | CRLF 行分隔符 `{0x0D, 0x0A}` |
...
| `ParseInvokeMessage(...)` | **abstract** | 子类实现协议解析（行 → `InvokeMessage[]`） |
| `ConvertResponseMessage(...)` | **abstract** | 子类实现响应序列化 |
| `ClientConnected` | 事件 | 客户端连接通知 |
| `ClientDisconnected` | 事件 | 客户端断开通知 |
| `ClientMessageInvoking` | 事件 | 消息拦截/修改/取消 |
...
| `Id` | `int` | 可选 | 消息唯一标识，用于请求-响应匹配，默认 0 |
| `Parameters` | `object[]` | 可选 | 方法参数列表，无参调用为 `null` |
| `ResponseMode` | `int` | 可选 | -1=不响应，0=默认，1=强制响应 |
...
InvokeMessage.Create("Video", "Seek", 5.6f);                          // 强类型参数
```
...
|------|------|------|
| `Id` | `int` | 对应请求消息的 Id |
| `Code` | `int` | 状态码：≥0 成功，<0 失败 |
| `ObjectMethod` | `string` | 被调用方法的完整名称 `{ObjectName}.{MethodName}` |
| `Description` | `string` | 结果描述或错误信息 |
| `ReturnType` | `Type` | 返回值类型 |
| `ReturnValue` | `object` | 返回值 |
| `Timestamp` | `DateTimeOffset` | 响应时间戳 |
| `Version` | `Version` | 协议版本 2.0.0 |

**状态码约定**：

| Code | 说明 |
|:----:|------|
| -1 | 对象未注册 (`ObjectNotRegistered`) |
| -2 | 方法被过滤/不允许调用 (`MethodFiltered`) |
| -3 | 方法不存在 (`MethodNotFound`) |
| -4 | 参数类型转换失败 (`ParameterConvertFailed`) |
| -5 | 方法反射调用异常 (`MethodInvokeException`) |
| 0 | 成功（void 方法，无返回值） |
| 1 | 成功（有返回值） |

### 3.4 `InvokeMessageEventArgs`

`ClientMessageInvoking` 事件的参数类，继承 `CancelEventArgs`。

通过设置 `Cancel = true` 可拦截取消本次调用；通过修改 `InvokeMessage` 属性可在执行前调整参数或响应模式。
...
**协议要点**：
- 以 CRLF 为行边界，以 `/>` 分隔行内的多个 XML 自闭合元素
- 一行可含多条消息，`ParseInvokeMessage` 内部逐条解析为 `InvokeMessage[]`
...
客户端占位类，当前未实现。客户端可直接使用 `TcpClient` + XML 字符串构造请求。

> 第三方团队参考：[远程过程调用(XML-RPC)消息协议.md](./远程过程调用(XML-RPC)消息协议.md)
...
```csharp
// 解析：行字节(ArraySegment<byte>) → InvokeMessage[]
protected abstract IEnumerable<InvokeMessage> ParseInvokeMessage(
    ArraySegment<byte> messageLine, IPEndPoint remoteEndPoint);

// 序列化：ResponseMessage → byte[] 响应数据
protected abstract byte[] ConvertResponseMessage(
    ResponseMessage responseMessage, IPEndPoint remoteEndPoint);
```
...
| 子类 | 消息格式 | 状态 |
|------|----------|:--:|
| `RPCServer4X` | XML（自闭合元素，`/>` 分隔）| ✅ 已实现 |
| `RPCServer4J` | JSON | 📋 规划中 |
...
关键约定：
- 参数支持弱类型 `@Parameters` 属性（自动类型转换）和强类型 `<Parameter Type="...">` 子节点
- 字符串用单/双引号包裹，数组用 `[...]`，十六进制用 `0x` 前缀
- `@ResponseMode` 控制是否响应：`-1` 不响应、`0` 默认、`1` 强制响应

---

## 5. 处理流程

### 5.1 全流程（连接 → 读 → 行拆分 → 消息解析 → 调用 → 响应）
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 750 620" width="100%" height="100%" style="max-width:750px;">
  ...
  <text x="350" y="54" text-anchor="middle" font-size="11" fill="#2b6cb0">Task.Run → AcceptTcpClientAsync() 循环</text>
  <text x="350" y="68" text-anchor="middle" font-size="9" fill="#718096">TcpListener.AcceptTcpClientAsync()</text>
  ...
  <text x="350" y="108" text-anchor="middle" font-size="11" fill="#276749" font-weight="bold">2. ReadTcpClientAsync(client)</text>
  <text x="350" y="124" text-anchor="middle" font-size="9" fill="#276749">环形缓冲 → 扫描 CRLF → 提取数据行</text>
  ...
  <text x="350" y="186" text-anchor="middle" font-size="11" fill="#975a16" font-weight="bold">3. ParseInvokeMessage(行) → InvokeMessage[]</text>
  <text x="350" y="202" text-anchor="middle" font-size="9" fill="#718096">子类实现：RPCServer4X 在行内以 /> 拆分多个 XML 消息</text>
  ...
  <text x="350" y="254" text-anchor="middle" font-size="10" fill="#4a5568">触发 ClientMessageInvoking 事件（可拦截/修改）</text>
  ...
  <text x="350" y="304" text-anchor="middle" font-size="10" fill="#4a5568">_ = CallInvokeMessageAsync(message)  ← Fire-and-Forget</text>
  ...
  <text x="350" y="352" text-anchor="middle" font-size="12" fill="#276749" font-weight="bold">4. CallInvokeMessageAsync 执行</text>
...
// 5. 可选：拦截消息
server.ClientMessageInvoking += (s, e) =>
{
...
// 发送调用请求
var request = "<InvokeMessage Id=\"1\" ObjectName=\"Demo\" MethodName=\"GetCurrentPage\" />\r\n";
...
// 输出: <ResponseMessage Id="1" Code="1" ObjectMethod="Demo.GetCurrentPage" ReturnValue="/home" ... />
```
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 450" width="100%" height="100%" style="max-width:700px;">
  ...
  <text x="25" y="70" font-size="9" fill="#4a5568">CallInvokeMessageAsync()</text>
...
`Send` 阻塞的 ThreadPool 线程量：`100 QPS × 2ms/次 = 0.2 个等效线程`，远低于上限。加上 `SemaphoreSlim(24, 60)` 保护，不会导致 ThreadPool 饥饿。
...
Fire-and-Forget 消除了轮询延迟，信号量替代了队列的背压能力。但在同一 `NetworkStream` 上需要关注并发写入安全（见 §8.1）。

### 7.3 为什么选择环形缓冲区？

- 避免频繁分配临时 `byte[]`，减少 GC 压力
- 支持粘包/半包场景：数据行可能跨越两次 `ReadAsync` 调用
- CRLF 扫描 + 紧凑化策略平衡了内存使用和性能

### 7.4 为什么预缓存 `MethodInfo`？

`RegisterObject` 时一次性扫描所有公共方法和扩展方法，生成 `{ObjectName}.{MethodName}({TypeSignature})` 缓存键。调用时直接 `TryGetValue`，避免每次调用都执行 `GetMethods()` + `GetParameters()` 的反射开销。
...
| # | 问题 | 位置 | 严重程度 | 修复建议 |
|:--:|------|------|:--:|------|
| 1 | `Stop()` 中 `catch` 未指定异常类型 | `RPCServerBase.cs:262,275` | 🟡 中 | 改为 `catch (Exception) when (IsDisposeRelated)` |
| 2 | 同一 `NetworkStream` 可能并发写入 | `RPCServerBase.cs:640` | 🟡 中 | 为每个 `TcpClient` 添加写入锁 |
| 3 | `TcpClient.Connected` 属性不可靠 | `RPCServerBase.cs:635` | 🔵 低 | 改用 Poll 检测真实连接状态 |
| 4 | `CacheInstanceMethods` 扫描所有程序集，重复开销大 | `RPCServerBase.cs:180` | 🟡 中 | 扩展方法缓存全局化 |

### 8.2 性能优化建议

| # | 建议 | 预期收益 |
|:--:|------|------|
| 1 | `ResponseMessage` / `InvokeMessage` 对象池化 | 减少 GC Alloc |
| 2 | 扩展方法缓存全局化（非 per-instance） | `RegisterObject` 延迟降低 90%+ |
| 3 | 考虑 `ArrayPool<byte>` 用于环形缓冲 | 减少大对象堆分配 |
| 4 | `SendBufferSize` / `ReceiveBufferSize` 可配置 | 适应不同网络环境 |

### 8.3 功能扩展方向

| # | 方向 | 说明 |
|:--:|------|------|
| 1 | `RPCServer4J` | JSON 协议的 RPC 服务端实现 |
| 2 | `RPCClient` 完整实现 | 连接管理、超时、重试、异步响应匹配 |
| 3 | 双向通知 | 服务端主动推送事件给客户端 |
| 4 | 连接认证 | Token / SharedSecret 机制 |
| 5 | 消息加密 | AES 加密（见协议文档） |
...
├── InvokeMessage.cs                      ← 请求消息数据类 + 事件参数
├── ResponseMessage.cs                    ← 响应消息数据类
├── RPCServerBase.cs                      ← RPC 服务端抽象基类
├── RPCServer4X.cs                        ← XML-RPC 服务端实现
└── RPCClient.cs                          ← TCP 扩展方法 + 客户端占位
```
...
| 方法名 | `GetCurrentPage`、`Seek`、`PlayPause` 等 |
| 方法缓存键 | `{ObjectName}.{MethodName}({TypeSignature})`，如 `Video.Seek(System.Single)` |
| 扩展方法标记 | 缓存键后缀 `_(Ext)`，如 `Demo.ToString()_(Ext)` |
