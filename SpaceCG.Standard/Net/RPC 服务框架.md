...
| **协议理念** | 格式可读性高，可编辑，可调试；以 CRLF 为行边界，一行一条消息 |
...
- **数据行**（基类统一处理）：TCP 字节流 → CRLF 拆分 → 环形缓冲 → 提取数据行
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 680 400" width="100%" height="100%" style="max-width:680px;">
  ...
  <text x="340" y="114" text-anchor="middle" font-size="10" fill="#718096">CRLF 行拆分（一行一条消息）</text>
...
- **信号量限流**：`SemaphoreSlim(25, 60)` 防止 ThreadPool 线程因 `Send` 阻塞而过度膨胀
...
| `DeserializeInvokeMessage(...)` | **abstract** | 子类实现协议解析（数据行 → `InvokeMessage`，单条请求消息） |
| `SerializeResponseMessage(...)` | **abstract** | 子类实现响应序列化（`ResponseMessage` → 数据行，单条响应消息） |
...
客户端抽象基类，与 `RpcServerBase` 镜像对称设计。提供连接管理（`Connect()` / `Close()`）、环形缓冲 CRLF 行拆分、请求/响应 Id 匹配、超时控制、自动重连等能力。<br />公共 API 包括 `InvokeFuncAsync()`（请求-响应，Func 语义）和 `InvokeActionAsync()`（单向通知，Action 语义）。
...
| 子类 | 消息格式 | 状态 |
|------|----------|:--:|
| `RpcServer4X` | XML（XML格式消息，每行一条）| ✅ 已实现 |
| `RpcServer4J` | JSON（JSON格式消息，每行一条） | 📋 规划中 |
