...
| -11 | 方法被过滤/禁止调用 |
| -10 | 注册对象不存在 |
| -3 | 调用请求被事件拦截取消（ClientInvokeRequest） |
| 0 | 成功（void 方法，无返回值） |
| 1 | 成功（有返回值） |

### 3.4 `InvokeMessageEventArgs`

`ClientInvokeRequest` 事件的参数类，继承 `CancelEventArgs`。

通过设置 `Cancel = true` 可拦截取消本次调用；可读取 `InvokeMessage` 属性查看调用详情。
...
客户端抽象基类，与 `RpcServerBase` 镜像对称设计。提供连接管理（`Connect()` / `Close()`）、环形缓冲 CRLF 行拆分、请求/响应 Id 匹配、超时控制、自动重连等能力。公共 API 包括 `InvokeFuncAsync()`（请求-响应，Func 语义）和 `InvokeActionAsync()`（单向通知，Action 语义）。
...
`RegisterObject` 时通过 `CacheObjectMethods` 一次性扫描所有公共方法和扩展方法，生成 `{ObjectName}.{MethodName}({TypeSignature})` 缓存键存入 `CacheMethodInfos`。调用时直接 `TryGetValue`，避免每次调用都执行 `GetMethods()` + `GetParameters()` 的反射开销。
...
| # | 问题 | 位置 | 严重程度 | 修复建议 |
|:--:|------|------|:--:|------|
| 1 | `Stop()` 中 `catch` 使用 `Exception` 基类 | `RpcServerBase.cs` | 🟡 中 | 改为更具体的异常类型 |
| 2 | 同一 `NetworkStream` 并发写入需信号量保护 | `RpcServerBase.cs` | ✅ 已解决 | 已为每个 TcpClient 添加 `ClientWriteSemaphores` |
| 3 | `TcpClient.Connected` 属性不可靠 | `RpcServerBase.cs` | ✅ 已解决 | 使用 `TcpClientExtensions.IsConnected()` Poll 检测 |
| 4 | `CacheObjectMethods` 扫描所有程序集，重复开销大 | `RpcServerBase.cs` | 🟡 中 | 扩展方法缓存全局化 |
| 5 | `Dispose()` 中 `ProcessInvokeSemaphore` 可能在活跃调用时抛出异常 | `RpcServerBase.cs` | 🔴 高 | 确保所有调用完成后再释放 |
