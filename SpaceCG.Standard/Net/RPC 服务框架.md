...
| `ResponseMode` | `int` | 可选 | -1=不响应，0=默认，1=必须响应 |
...
客户端抽象基类，与 `RpcServerBase` 镜像对称设计。提供连接管理、环形缓冲 CRLF 行拆分、请求/响应 Id 匹配、超时控制、自动重连等能力。公共 API 包括 `InvokeFuncAsync()`（请求-响应，Func 语义）和 `InvokeActionAsync()`（单向通知，Action 语义）。
