...
| **协议理念** | XML 格式可读性高，可编辑，可调试；以 CRLF 为行边界，一行一条消息 |
...
基类 `RpcServerBase` 以 **CRLF 数据行** 作为行单位，一个数据行对应一条消息：
...
- **消息层**（子类协议实现）：数据行字节 → `DeserializeInvokeMessage()` → `InvokeMessage`（单条）
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 680 400" width="100%" height="100%" style="max-width:680px;">
  ...
  <text x="340" y="114" text-anchor="middle" font-size="10" fill="#718096">CRLF 行拆分（一行一消息）</text>
...
| **行解析层** | 环形缓冲 + CRLF 扫描 | 字节流 → CRLF 行拆分（基类统一实现，一行一条消息） |
| **协议解析层** | `DeserializeInvokeMessage()` (abstract) | 行字节 → `InvokeMessage`（子类实现，单条消息） |
...
| `DeserializeInvokeMessage(...)` | **abstract** | 子类实现协议解析（行 → `InvokeMessage`，单条消息） |
| `SerializeResponseMessage(...)` | **abstract** | 子类实现响应序列化 |
...
客户端抽象基类，与 `RpcServerBase` 镜像对称设计。提供连接管理、环形缓冲 CRLF 行拆分、请求/响应 Id 匹配、超时控制、自动重连等能力。公共 API 包括 `InvokeAsync()`（请求-响应）和 `NotifyAsync()`（单向通知）。
...
```csharp
// 解析：行字节(ArraySegment<byte>) → InvokeMessage（单条消息）
protected abstract InvokeMessage DeserializeInvokeMessage(
    ArraySegment<byte> dataLine, IPEndPoint remoteEndPoint);

// 序列化：ResponseMessage → byte[] 响应数据
protected abstract byte[] SerializeResponseMessage(
...
| 子类 | 消息格式 | 状态 |
|------|----------|:--:|
| `RpcServer4X` | XML（自闭合元素，每行一条）| ✅ 已实现 |
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 750 620" width="100%" height="100%" style="max-width:750px;">
  ...
  <text x="350" y="202" text-anchor="middle" font-size="9" fill="#718096">子类实现：一行对应一条消息，由子类解析为 InvokeMessage</text>
...
| 5 | 类文档注释仍提"一行可含多条消息"，与实际单消息模式不一致 | `RpcServerBase.cs:28` | 🔵 低 | 更新 XML 注释 |
...
| # | 方向 | 说明 |
|:--:|------|------|
| 1 | `RpcServer4J` + `RpcClient4J` | JSON 协议的 RPC 服务端+客户端实现 |
| 2 | `RpcClient4X` 完善 | XML 客户端序列化/反序列化实现 |
...
├── InvokeMessage.cs                      ← 请求消息数据类 + 事件参数 + 对象池
...
> 文档版本：v2.1  |  最后更新：2026-07-12  |  维护：SpaceCG 团队
