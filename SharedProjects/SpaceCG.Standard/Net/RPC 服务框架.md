...
| **协议理念** | 格式可读性高，可编辑，可调试；默认以 CRLF 作为字节数据分隔符，对应一条数据消息 |
...
- **数据拆分层**（基类统一处理）：TCP 字节流 → 字节拆分 → 环形缓冲 → 提取字节消息
- **协议解析层**（子类协议实现）：数据消息字节 → `DeserializeInvokeMessage()` → `InvokeMessage`（单条）
...
| **数据拆分层** | 环形缓冲 + 分割符扫描 | 字节流 → 字节拆分（基类统一实现） |
| **协议解析层** | `DeserializeInvokeMessage()` (abstract) | 数据字节 → `InvokeMessage`（子类实现，单条消息） |
