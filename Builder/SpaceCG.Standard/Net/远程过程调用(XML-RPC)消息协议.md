...
| 2.0 | 2026-07 | **重构**：数据消息默认按行拆分，简化协议实现，XML 不支持子节点，优化缓存，提高序列与反序列性能 |
...
- 传输数据以 XML 元素结束标记 `/>`（0x2F, 0x3E）为消息分隔符，每次从 TCP 流中读取一个完整的 XML 消息
- 一个 XML 元素对应一条消息（v2.0 起，一个元素对应一条消息）
...
|------|------|
| 传输协议 | TCP（可靠字节流） |
| 数据定界 | XML 元素结束标记 `/>`（`0x2F 0x3E`）|
| 消息格式 | XML 自闭合元素格式，以 `/>` 结束，尾部附带 `\r\n` 换行 |
| 字符编码 | UTF-8 |
| 连接模式 | 长连接，单连接可发送多行 |
...
1. **数据定界**：以 XML 元素结束标记 `/>`（`0x2F 0x3E`）拆分消息。TCP 粘包/半包需自行处理，建议使用缓冲区拼接。**每个 XML 元素一条消息**。
